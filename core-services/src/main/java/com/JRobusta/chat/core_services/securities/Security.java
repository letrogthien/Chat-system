...
public class Security {
  ...
  private static List<String> allowedOrigins =
      List.of("https://auth.wezd.io.vn", "https://admin.wezd.io.vn", "https://wezd.io.vn",
  ...
  private void configureOAuth2ResourceServer(HttpSecurity security) throws Exception {
    security.oauth2ResourceServer(
        oauth2 -> oauth2.jwt(jwt -> jwt.decoder(jwtDecoder).jwtAuthenticationConverter(converter))
            .bearerTokenResolver(getTokenResolver).authenticationEntryPoint(entryPoint));
  }
  ...
  private void configureAuthorizationRules(HttpSecurity security) throws Exception {
    security.authorizeHttpRequests(authz -> authz.anyRequest().permitAll());
  }
