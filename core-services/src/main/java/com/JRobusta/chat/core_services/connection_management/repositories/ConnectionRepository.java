...
public class ConnectionRepository {
  ...
  public void saveSetConnectionByGatewayNodeId(String gatewayNodeId, String connectionId) {
    redisTemplate.opsForSet().add(Const.CONNECTION_BY_GATEWAY.getValue() + gatewayNodeId,
        connectionId);
  }

  public void deleteSetConnectionInGatewaySet(String gatewayNodeId, String connectionId) {
    redisTemplate.opsForSet().remove(Const.CONNECTION_BY_GATEWAY.getValue() + gatewayNodeId,
        connectionId);
  }

  public void deleteSetConnection(String userId, String connectionId) {
    redisTemplate.opsForSet().remove(Const.CONNECTION_VERIFIED_SET.getValue() + userId,
        connectionId);
  }
  ...
  public List<ConnectionManagerOuterClass.Connection> getConnections(String userId) {
    ...
    return Objects
        .requireNonNull(
            redisTemplate.opsForSet().members(Const.CONNECTION_VERIFIED_SET.getValue() + userId))
        .stream().map(connectionId -> {
  ...
  public Set<Object> getConnectionByGatewayNodeId(String gatewayNodeId) {
    return redisTemplate.opsForSet()
        .members(Const.CONNECTION_BY_GATEWAY.getValue() + gatewayNodeId);
  }
  ...
  public List<ConnectionManagerOuterClass.Connection> getAllConnections() {
    Set<String> keys = redisTemplate.keys(Const.CONNECTION_VERIFIED_HASH + "*");
    if (keys.isEmpty()) {
      return Collections.emptyList();
    }

    List<ConnectionManagerOuterClass.Connection> connections = new ArrayList<>();
    for (String key : keys) {
      Map<Object, Object> entries = redisTemplate.opsForHash().entries(key);
      if (!entries.isEmpty()) {
        connections.add(getConnection(entries));
      }
    }
    return connections;
  }
  ...
  private ConnectionManagerOuterClass.Connection getConnection(Map<Object, Object> entries) {
    return ConnectionManagerOuterClass.Connection.newBuilder()
        ...
        .setConnectedAt(com.google.protobuf.util.Timestamps
            .fromMillis(Long.parseLong(entries.get("connected_at").toString())))
        .setLastPingAt(com.google.protobuf.util.Timestamps
            .fromMillis(Long.parseLong(entries.get("last_ping_at").toString())))
