...
public class DebeziumConfig {
  ...
  public static Configuration debeziumConnectorConfig(
    ...
    return Configuration.create().with("name", "message-outbox-connector")
        .with("connector.class", "io.debezium.connector.mysql.MySqlConnector")
        .with("offset.storage", "org.apache.kafka.connect.storage.FileOffsetBackingStore")
        .with("offset.storage.file.filename", "F:/chat-sys/tmp/offsets.dat")
        .with("offset.flush.interval.ms", "60000").with("database.hostname", hostname)
        .with("database.port", port).with("database.user", datasourceUsername)
        .with("database.password", datasourcePassword).with("database.server.id", "85744")
        .with("topic.prefix", "dbserver1").with("database.include.list", database)
        ...
        .with("database.allowPublicKeyRetrieval", "true").with("include.schema.changes", "false")
        .build();
  }
  ...
  public void buildEngine() {
    this.debeziumEngine = DebeziumEngine.create(ChangeEventFormat.of(Connect.class))
        .using(debeziumConfiguration.asProperties()).notifying(this::handleChangeEvent).build();
  }
  ...
  private void handleChangeEvent(RecordChangeEvent<SourceRecord> event) {
    try {
      ...
      sendEventService.sendEventOutbox(KafkaTopic.OUTBOX_EVENT.getTopicName(), uuid + "|" + payload,
          conversationId).whenComplete((result, ex) -> {
            if (ex != null) {
              System.out.println(
                  "Failed to send outbox event to Kafka, falling back to Redis for outboxId: "
                      + uuid);
